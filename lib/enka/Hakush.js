...
async function fetchJson(url) { // Accept optional logger
...
async function fetchAndSaveEquipment(outputDir) {
    logger.info('[HakushUpdater] --- Fetching Equipment Data ---');
    const equipData = await fetchJson(API_ENDPOINTS.ALL_EQUIP);
    if (equipData) {
        const outputFile = path.join(outputDir, 'equipment.json');
        await saveJson(outputFile, equipData);
        return true;
    } else {
...
async function fetchAndSaveCharacters(outputDir) {
    logger.info('[HakushUpdater] --- Fetching Character Data ---');
    const allCharInfo = await fetchJson(API_ENDPOINTS.ALL_CHAR);
    if (!allCharInfo) {
    ...
    for (const charId of charIds) {
        ...
        const charDetail = await fetchJson(detailUrl);
        if (charDetail) {
            const outputPath = path.join(charOutputDir, `${charId}.json`);
            await saveJson(outputPath, charDetail);
            successCount++;
        } else {
...
async function fetchAndSaveWeapons(outputDir) {
    logger.info('[HakushUpdater] --- Fetching Weapon Data ---');
    const allWeaponInfo = await fetchJson(API_ENDPOINTS.ALL_WEAPON);
    if (!allWeaponInfo) {
    ...
    for (const weaponId of weaponIds) {
        ...
        const weaponDetail = await fetchJson(detailUrl);
        if (weaponDetail) {
            const outputPath = path.join(weaponOutputDir, `${weaponId}.json`);
            await saveJson(outputPath, weaponDetail);
            successCount++;
        } else {
 ...
 */
export async function updateHakushData(outputDirectory = null = console) {
    ...
    try {
        ...
        // Sequential example:
        const equipSuccess = await fetchAndSaveEquipment(outputDirectory);
        const charSuccess = await fetchAndSaveCharacters(outputDirectory);
        const weaponSuccess = await fetchAndSaveWeapons(outputDirectory);
